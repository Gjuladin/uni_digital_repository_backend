...
```
docker build -t dspace/dspace-dependencies:dspace-10.0 -f Dockerfile.dependencies .
```
...
```
docker push dspace/dspace-dependencies:dspace-10.0
```
...
```
docker build -t dspace/dspace:dspace-10.0-test -f Dockerfile.test .
```
...
Admins to our DockerHub repo can manually publish with the following command.
```
docker push dspace/dspace:dspace-10.0-test
```
...
```
docker build -t dspace/dspace:dspace-10.0 -f Dockerfile .
```
...
```
docker push dspace/dspace:dspace-10.0
```
...
```
docker build -t dspace/dspace-cli:dspace-10.0 -f Dockerfile.cli .
```
...
Admins to our DockerHub repo can publish with the following command.
```
docker push dspace/dspace-cli:dspace-10.0
```
...
docker build -t dspace/dspace-postgres-loadsql:dspace-10.0 .
```

It is also possible to change the version of PostgreSQL or the PostgreSQL user's password during the build:
```
cd dspace/src/main/docker/dspace-postgres-loadsql
docker build -t dspace/dspace-postgres-loadsql:dspace-10.0 --build-arg POSTGRES_VERSION=17 --build-arg POSTGRES_PASSWORD=mypass .
```

Admins to our DockerHub repo can (manually) publish with the following command.
```
docker push dspace/dspace-postgres-loadsql:dspace-10.0
```
